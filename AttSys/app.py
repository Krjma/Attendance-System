...
def student_login():
    if request.method == 'POST':
        ...
        if student:
            ...
            flash('Invalid Student ID or Password. Please try again.')
...
def instructor_login():
    if request.method == 'POST':
        ...
        # Validate instructor ID and password
        conn = get_db()
        instructor = conn.execute('SELECT * FROM Instructors WHERE id = ? AND password = ?', (instructor_id, password)).fetchone()
        conn.close()
        ...
        if instructor:
            ...
            flash('Invalid Instructor ID or Password. Please try again.')
...
def advisor_login():
    if request.method == 'POST':
        ...
        # Validate advisor ID and password
        conn = get_db()
        advisor = conn.execute('SELECT * FROM Advisors WHERE id = ? AND password = ?', (advisor_id, password)).fetchone()
        conn.close()
        ...
        if advisor:
            ...
            flash('Invalid Advisor ID or Password. Please try again.')
